...
public class MemberUtils {
    ...
    public static void onFail(Member member, Throwable ex) {
        ...
        if (cloneMember.getFailAccessCnt() > maxFailAccessCnt || StringUtils.containsIgnoreCase(ex.getMessage(), TARGET_MEMBER_CONNECT_REFUSE_ERRMSG)) {
    ...
    static boolean fullEquals(Member actual, Member expected) {
        if (null == expected) {
            return null == actual;
        }
