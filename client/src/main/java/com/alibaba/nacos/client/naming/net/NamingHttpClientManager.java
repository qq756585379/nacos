...
public class NamingHttpClientManager implements Closeable {
    ...
    public void shutdown() throws NacosException {
        ...
        try {
            ...
            NAMING_LOGGER.error("[NamingHttpClientManager] An exception occurred when the HTTP client was closed : {}", ExceptionUtil.getStackTrace(ex));
        }
    ...
    private static class NamingHttpClientFactory extends AbstractHttpClientFactory {
        ...
        protected HttpClientConfig buildHttpClientConfig() {
            return HttpClientConfig.builder().setConTimeOutMillis(CON_TIME_OUT_MILLIS).setReadTimeOutMillis(READ_TIME_OUT_MILLIS).setMaxRedirects(MAX_REDIRECTS).build();
        }
